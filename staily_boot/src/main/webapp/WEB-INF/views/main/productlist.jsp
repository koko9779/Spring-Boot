...
<!-- Section -->
<div class="container-fluid section news totop" style="padding: 0 10%;">
   <div id="afterHeader">
      <article style="width:100%">
         <c:forEach var="pList" items="${pList}" varStatus="status">
            <c:choose>
               <c:when test="${(status.count%3) eq 2}">
                  <form id="product_${pList.product[0].pNo}" class="product_form_size" style="margin: 0 4%;">
               </c:when>
               <c:otherwise>
                  <form id="product_${pList.product[0].pNo}" class="product_form_size">               
               </c:otherwise>
            </c:choose>
